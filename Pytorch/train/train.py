...
ckpt_path = './ckpt'
#ckpt_path = '..\\ckpt'
exp_name = 'mapillary-fcn32s'
writer = SummaryWriter(os.path.join(ckpt_path, 'exp', exp_name))

PATH = './mapillary_fcn32s.pth'
...
def main():
    ...
    open(os.path.join(ckpt_path, exp_name, str(datetime.datetime.now()).replace(':', '-') + '.txt'), 'w').write(str(args) + '\n\n')
...
def validate(val_loader, net, criterion, optimizer, epoch, train_args, restore, visualize):
    ...
    acc, acc_cls, mean_iu, fwavacc = evaluate(predictions_all, gts_all, mapillary.num_classes)
    ...
    if mean_iu > train_args['best_record']['mean_iu']:
        ...
        for idx, data in enumerate(zip(inputs_all, gts_all, predictions_all)):
            ...
            gt_pil = mapillary.colorize_mask(data[1])
            predictions_pil = mapillary.colorize_mask(data[2])
            if train_args['val_save_to_img_file']:
